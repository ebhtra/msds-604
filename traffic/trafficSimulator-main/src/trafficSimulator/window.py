...
class Window:
    ...
    BKGRD = (200, 200, 200)  # background color greyish
    ...
    def loop(self, loop=None):
        ...
        while running:
            ...
            clock.tick(self.fps)  # small clock display in upper left of viz window

            # Handle all events --  Only using zooming and panning for the stop signs project
    ...
    def rotated_box(self, pos, size, angle=None, cos=None, sin=None, centered=True, color=FOREST, filled=True):
    ...
    def rotated_rect(self, pos, size, angle=None, cos=None, sin=None, centered=True, color=FOREST):
    ...
    def draw_vehicle(self, vehicle, road):
        ...
        self.rotated_box((x, y), (l, h), cos=cos, sin=sin, centered=True, color=vehicle.clr)
        
        # add blinker where needed for viz/debugging
        if vehicle.queued:
            if vehicle.direction == 'RIGHT':
                self.rotated_box((x-sin, y+cos), (1,1), cos=cos, sin=sin, color=(255,0,0))
    ...
    def draw_signals(self):
        for signal in self.sim.traffic_signals:
            for i in range(len(signal.roads)):
                # I'm making them all red, for 4-way stop sign:
                #original:  color = (0, 255, 0) if signal.current_cycle[i] else (255, 0, 0)
                color = (255, 0, 0)
                for road in signal.roads[i]:
                    position = (  
                        road.end[0] - 4 * road.angle_sin,        
                        road.end[1] + 4 * road.angle_cos
                    )
                    self.rotated_box(
                        position,
                        (1, 3),
